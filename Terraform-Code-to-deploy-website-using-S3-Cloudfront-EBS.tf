...
resource "aws_s3_bucket" "ashokdevops8s3" {
  bucket = "ashokdevops8s3"
 ...
 provisioner "local-exec" {
    command = "git clone https://github.com/ashokdevops8/The-Delta.git web-server-image"
 }
...
resource "aws_s3_bucket_public_access_block" "public_storage" {
 depends_on = [aws_s3_bucket.ashokdevops8s3]
 bucket = "ashokdevops8s3"
...
resource "aws_cloudfront_distribution" "tera-cloufront1" {
    ...
    origin {
        domain_name = aws_s3_bucket.ashokdevops8s3.bucket_regional_domain_name
